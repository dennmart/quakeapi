...
describe "quakeapi" do
  ...
  describe "GET /earthquakes.json" do
    before(:all) do
      stub_request(:get, Earthquake::INFO_URL).to_return(:body => File.new(File.dirname(__FILE__) + "/fixtures/additional_earthquake_response.txt"))
    ...
    it "returns all earthquakes if no parameters are passed" do
      ...
      earthquakes = JSON.parse(last_response.body)
    ...
    it "returns all earthquakes from a specific date if params['on'] is passed" do
      ...
      ids = earthquakes.map { |quake| quake["earthquake_id"] }
      ids.should include(1, 2)
    ...
    it "returns all earthquakes since a specific date if params['since'] is passed" do
      ...
      ids = earthquakes.map { |quake| quake["earthquake_id"] }
      ids.should include(1, 3)
      ids.should_not include(2)
    end
    ...
    it "returns all earthquakes over a certain magnitude if params['over'] is passed" do
      ...
      ids = earthquakes.map { |quake| quake["earthquake_id"] }
      ids.should include(2, 3)
      ids.should_not include(1)
    end
